...
④ 给出或检查 python main.py --netlist <circuit_name>/<circuit_name>.cir --testbench <circuit_name>/tb_<circuit_name>_ac.scs [tb_sr.scs] [tb_st.scs] --requirements <circuit_name>/requirements.json
   ...
   **只传 SR/ST testbench：仅当用户需求中包含摆率或建立时间指标时**
      │
      ▼
⑤ 用户完成真实仿真后，Codex 按优先级读取结果：
   outputs/<project_name>/results.json
   → outputs/<project_name>/optimization_metrics.csv
   → outputs/<project_name>/optimization_log.json
   → workspace/run_xxx/diagnostics/diagnostics_summary.txt
...
4. **[pdk_profiles.py](Agent_LLM_BO/circuit_agent/pdk_profiles.py)** — 代码实际使用的 PDK 分组：Spectre/HSPICE model 路径、section、NMOS/PMOS/LVT model 名称、默认 VDD、VDD 允许范围、gm/Id 表路径、PVT 温度、Spectre options、Virtuoso tech library，以及各 topology 的工艺专用初始参数 preset
5. **[topologies/*_optimization.md](knowledge_base/Opamp_knowledge_base/topologies/)** — 每个拓扑的结构、参数含义、DC OP 诊断和调参规则；BO 后 Review 时优先结合当前拓扑专用文件。

> 不要在 topology 文件中硬编码 PDK 路径、MOS model 名称、电源默认值或某工艺专用初始 W/L；需要换工艺时，新增/选择 `PDKProfile`，或用 `.env` / `PDK_PROFILE_FILE` 覆盖 `PDK_SPECTRE_PATH`、`NMOS_MODEL`、`PMOS_MODEL`、`NMOS_LVT_MODEL`、`PMOS_LVT_MODEL`、`GMID_TABLE_PATH`、`VIRTUOSO_TECH_LIB` 等字段。
...
> 初始参数由 `topology.DEFAULT_PARAMS` 和当前 PDK profile 的 `topology_presets` 共同决定：profile preset 的 `default_params` 覆盖初始网表参数，`testbench_defaults` 覆盖 VCM/IBIAS/CL/VBIAS 等 testbench 默认值，`param_space_overrides` 覆盖 BO 搜索范围。`topology_presets` 是可选校准层，不要求每个 PDK 为每个 topology 都填写；换工艺或换器件型号后，只有某拓扑初始工作点/搜索范围不合适时才补 preset，并且优先改 profile preset，不要直接改 topology 源码默认尺寸。
...
> **层级系统例外**：`bandgap_ptat` 是系统级 topology，不应把 folded-cascode 内部 W/L 展开到 bandgap 级 BO。先单独优化/验证 folded-cascode child opamp，再将其 `circuit.cir` 作为 frozen macro 嵌入 bandgap；bandgap 级 BO 只调 resistor/current/ratio/pass-device/compensation 等系统参数。规则见 `knowledge_base/Opamp_knowledge_base/topologies/bandgap_ptat_optimization.md`。
...
- 负增益、缺失关键 AC 指标、连续严重偏差会触发重罚或早停，避免在明显不可用工作点上浪费迭代。
- PM 达标后仍不是越大越好；当前 reward 会对过大的相位裕度（例如高于 75°）施加惩罚，避免过补偿牺牲 GBW/SR。
- DC 工作点已经接入 reward：优化器读取每轮 `workspace/run_xxx/diagnostics/dc_operating_points.csv`，用 `|vds|-|vdsat|` 判断 critical MOS 是否 linear/near-edge。critical 主路径器件进线性区会强扣分；noncritical/bias 器件只记录 warning，不参与 reward 扣分。若性能指标全满足但 critical MOS 仍在线性区，BO 不会提前停止。
- `optimization_metrics.csv` 会包含 `op_linear_count`、`op_near_edge_count`、`op_min_margin_mv`；`results.json`/`optimization_log.json` 会记录 `operating_point_status`。遇到 folded cascode 等复杂拓扑的负增益或 PVT 崩溃，优先分析每轮 `diagnostics_summary.txt` 和 OP 统计。

### folded cascode bias 与 OP-guided Review

- folded cascode 的 bias generator 结构仍保持固定，但 BO/gmId pass-through 允许小范围调节 `bias_p_scale`、`bias_n_scale`、`bias_p_small_scale`、`bias_n_small_scale`，用于微调 VB1-4 相关 bias 强度；不要把它理解为重新开放所有 bias W/L。
- BO 后不提供 `patch_plan.json` 时，`review_optimization.py` 的内置 fallback 会额外读取 folded run 的 `diagnostics/dc_operating_points.csv`。若 `|vds|-|vdsat| < 50mV`，会生成保守候选：
  - `Mtailp/Mmirr/Mcasp` 问题 → 增大 `bias_p_scale`
  - `Mfold/Mcasn/Mload` 问题 → 增大 `bias_n_scale`
  - `M6/M25` 问题 → 分别增大 `bias_p_small_scale` / `bias_n_small_scale`
  - `Mcs` 问题 → 增大 `Wcs`
  - `Mdiff1/2` 问题 → 增大 `Wdiffp`
- 这只是候选修复，不代表一定变好；必须运行 review candidate 仿真，并比较 `candidate_metrics.csv`、`metrics_summary.txt` 和 diagnostics。

---

## 达标后导出 Virtuoso 原理图

当优化结果达到指标后，下一步是把最终 netlist 导出为 Virtuoso schematic。Codex 应优先使用 `export_to_virtuoso.py --results`，不要手动挑文件改 SKILL。

选择规则：

- 如果 `outputs/<project>/agent_review/candidate_metrics.csv` 中存在达标 candidate，导出 Review candidate 对应的 `circuit.cir`
- 否则导出 BO 最优 `outputs/<project>/netlist/circuit.cir`
- 导出来源会写入 `outputs/<project>/virtuoso/export_report.json` 的 `export_source`

默认命令只生成 SKILL，不启动 Cadence：
...
如果用户明确希望自动创建 Cadence 工作目录并尝试导入，才使用：
...
`run_import.il` 负责创建/打开 `BO_Designs` library、绑定 `tsmcN28` tech library，并加载 `import_schematic.il` 生成 schematic。Codex 默认不在本机运行 Virtuoso/Cadence；如果用户只需要文件，停在 SKILL/workspace 生成即可。

batch Virtuoso 排错优先级：

- `CDS.log File is already locked`：已有 GUI 进程占用默认 `~/CDS.log`。导出器默认设置 `CDS_LOG=<virtuoso-workdir>/CDS.log`，必要时用 `--cds-log <path>` 显式覆盖。
- `Tech library tsmcN28 is not visible`：batch 进程没有加载用户/站点 `cds.lib` 或 PDK OA library。优先使用 `--include-cds-lib /home/userone/cds.lib`；如果仍不可见，再加 `--pdk-lib-path /PDKS/TSMC28nm/tsmcN28`，使生成的 `cds.lib` 包含 `DEFINE tsmcN28 ...`。

> 当前导出是通过 SKILL 脚本创建 schematic，不做 layout，不自动跑 ADE。`/PDKS/TSMC28nm/models/spectre/toplevel.scs` 是 Spectre 模型 include，不是 Virtuoso tech library；tech library 名称用 `--tech-lib` 指定。
...
Python 脚本不再尝试自动修补网表（网表由拓扑库生成，语法正确）。真实仿真失败通常是收敛问题、参数极端值、PDK 路径或 Cadence 环境导致：
...
• 根据用户提供的结果继续分析
• 达标后生成 Virtuoso SKILL/工作区
```

> **外部 LLM 的角色已收敛为可选能力**：主 BO loop 默认不调用外部 LLM，也不需要 `DEEPSEEK_API_KEY`。核心优化由拓扑硬约束、gm/Id sizing、Optuna BO、Spectre parser、diagnostics/OP penalty 和本地 Agent Review 驱动。`llm_client.py` 仅保留自然语言需求解析、实验性参数校验或模糊拓扑选择；只有显式启用 `enable_llm_validation=True` 时，BO 迭代才会调用 `validate_and_adjust_params()`。
...
- **拓扑专用优化知识库**：[knowledge_base/Opamp_knowledge_base/topologies/](knowledge_base/Opamp_knowledge_base/topologies/)
