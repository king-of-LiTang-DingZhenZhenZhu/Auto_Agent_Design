...
class SpectreTopologyTest(unittest.TestCase):
    ...
    def test_bandgap_ptat_embeds_frozen_two_stage_ota_macro(self):
        ...
        self.assertIn("MX (vinn sup vdd vdd)", circuit)
        self.assertIn("MY (vinp sup vdd vdd)", circuit)
        self.assertIn("parameters Wstart_y=300n Wstart_x=600n", circuit)
        self.assertIn("QRS (vss vss vrs_diode)", circuit)
        self.assertIn("MSTART_BIAS (start_bias start_bias vdd vdd)", circuit)
        self.assertIn("RSTART_BIAS (start_bias vss) resistor r=Rstart_bias", circuit)
        self.assertIn("MSTART_VRS (vrs_diode start_bias vdd vdd)", circuit)
        self.assertIn("MSTART_CMP (cmp_tail start_bias vdd vdd)", circuit)
        self.assertNotIn("IVRSBIAS", circuit)
        self.assertNotIn("ICMPBIAS", circuit)
        self.assertIn("RRS_TOP (vrs_diode vrs) resistor r=Rstart_ref", circuit)
        self.assertIn("RRS_BOTTOM (vrs vss) resistor r=0.9*Rstart_ref", circuit)
    ...
    def test_bandgap_ptat_generates_dedicated_signoff_testbenches(self):
        ...
        self.assertIn("save Xdut.vrs Xdut.sup Xdut.cmp_out", startup)
