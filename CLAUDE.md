...
③ 调 Python 拓扑库生成网表文件（硬约束，语法保证正确）
   ...
   ├── tb_<circuit_name>_tran.scs   # （可选）Transient 仿真 testbench
   ├── params.json                 # （可选）参数搜索空间，省略时自动从网表提取
   └── requirements.json           # 设计指标
      │
      ▼
④ 确认已激活 Auto_Agent_Design 环境，调用 python main.py --netlist <circuit_name>/<circuit_name>.cir --testbench <circuit_name>/tb_<circuit_name>_ac.scs <circuit_name>/tb_<circuit_name>_tran.scs --requirements <circuit_name>/requirements.json
...
#   ├── tb_5t_ota_tran.scs       # （如果拓扑支持）Transient testbench
