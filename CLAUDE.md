...
③ 调 Python 拓扑库生成网表文件（硬约束，语法保证正确）
   ...
   └── requirements.json           # 设计指标
      │
      ▼
④ 确认已激活 Auto_Agent_Design 环境，调用 python main.py --netlist <circuit_name>/<circuit_name>.cir --testbench <circuit_name>/tb_<circuit_name>_ac.sp <circuit_name>/tb_<circuit_name>_tran.sp --requirements <circuit_name>/requirements.json
