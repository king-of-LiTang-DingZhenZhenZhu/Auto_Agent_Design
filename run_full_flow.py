...
def main() -> int:
    ...
    if starting_from_requirements and physical_requested and not (args.run_pvt and args.simulate):
        raise SystemExit(
            "a new full physical flow requires --run-pvt --simulate so PVT evidence is real"
        )
    ...
    if physical_requested and topology:
    ...
    successful_actions = {"done", "ready_to_export_virtuoso", "run_signoff"}
